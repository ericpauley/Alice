...
public class KeepTouchingAnimation extends AbstractBodyPositionAnimation {
	...
	// offsetDirection = new
	// edu.cmu.cs.stage3.alice.core.property.DirectionProperty(this,
	...
	public class RuntimeTouchAnimation extends RuntimeAbstractBodyPositionAnimation {
		...
		public void prologue(double t) {
			 ...
			 * lowerLimb.getBoundingBox(lowerLimb).getCenterOfBottomFace(); tmp
		...
		/*
		 * edu.cmu.cs.stage3.math.Vector3 getTargetPosition() { // this is the
		 * goal position in the target's space - we need to know where it is
		...
		protected void setTargetQuaternions() {
			...
			if (targetDistance < upperLimbLength + lowerLimbLength) {
				...
				if (!Double.isNaN(limbAngle)) {
					...
					// arms turn backwards, legs turn forwards so as not to
					// break people
					// upper limbs turn 1/2 limbAngle so that effective arm is
					// still along y axis
					if (limb.getLimbValue().equals(Limb.leftArm) || limb.getLimbValue().equals(Limb.rightArm)) {
						if (lowerLimb != null) {
