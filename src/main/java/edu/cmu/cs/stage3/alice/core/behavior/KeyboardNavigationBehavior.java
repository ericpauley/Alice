...
public class KeyboardNavigationBehavior extends InternalResponseBehavior {
	...
	public void internalSchedule(double time, double dt) {
		...
		/*
		 * double yPos =
		 * ((Transformable)subject.get()).getPosition().getItem(1);
		 * ((Transformable
		 * )subject.get()).moveRightNow(Direction.FORWARD,dt*speed.z); if
		 * (((Boolean)stayOnGround.get()).booleanValue()) { Vector3 pos =
		 * ((Transformable)subject.get()).getPosition(); pos.setItem(1,yPos);
		 * ((Transformable)subject.get()).setPositionRightNow(pos); }
		 * ((Transformable
		 * )subject.get()).moveRightNow(Direction.RIGHT,dt*speed.x);
