...
public class TouchAnimation extends AbstractBodyPositionAnimation {
	...
	public class RuntimeTouchAnimation extends RuntimeAbstractBodyPositionAnimation {
		...
		/*public void epilogue( double t ) {
			super.epilogue( t );
			javax.vecmath.Vector3d tmp = lowerLimb.getBoundingBox(lowerLimb).getCenterOfBottomFace();
			
			
			tmp = lowerLimb.getPosition(tmp, m_target);
		
			System.out.println("goal vector: " + tmp);
			System.out.println("m target vector: " + m_target.getPosition(tmp, upperLimb));
...
//				arms turn backwards, legs turn forwards so as not to break people
				...
				if ( (limb.getLimbValue().equals(Limb.leftArm)) || (limb.getLimbValue().equals(Limb.rightArm)) ) {
					if (lowerLimb != null) lowerLimb.turnRightNow(edu.cmu.cs.stage3.alice.core.Direction.BACKWARD, limbAngle);	
					upperLimb.turnRightNow(edu.cmu.cs.stage3.alice.core.Direction.FORWARD, (limbAngle / 2.0));
				} else {
					if (lowerLimb != null) lowerLimb.turnRightNow(edu.cmu.cs.stage3.alice.core.Direction.FORWARD, limbAngle);
