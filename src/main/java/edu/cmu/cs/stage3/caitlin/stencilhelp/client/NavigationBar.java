...
public class NavigationBar implements StencilObject, MouseEventListener, StencilStackChangeListener, LayoutChangeListener {
	...
	protected void generateShapes() {
		...
		// underClose = new RoundRectangle2D.Double(topCenter.x + 110+2,
		// topCenter.y+2, 20 ,25, 10, 10);
	...
	public boolean mouseClicked(MouseEvent e) {
		if (bgNext.contains(e.getPoint()) && stencilManager.hasNext()) {
			stencilManager.showNextStencil();
														...
														// (bgRestart.contains(e.getPoint()))
														// &&
														// (stencilManager.hasPrevious())
														// ) {
	...
	protected void updateNavBar() {
		/*
		 * if (stencilManager.hasNext()) { ScreenShape s =
		 * (ScreenShape)shapes.elementAt(5); s.setColor(Color.blue); } else {
		 * ScreenShape s = (ScreenShape)shapes.elementAt(5);
		 * s.setColor(Color.lightGray); } if (stencilManager.hasPrevious()){
		 * ScreenShape s = (ScreenShape)shapes.elementAt(4);
		 * s.setColor(Color.blue); } else { ScreenShape s =
		 * (ScreenShape)shapes.elementAt(4); s.setColor(Color.lightGray); }
