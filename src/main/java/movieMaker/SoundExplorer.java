...
public class SoundExplorer implements MouseMotionListener, ActionListener, MouseListener, LineListener {
	 ...
	 * @param enabled
	 *            if true set button enabled else disable it
	 * @param panel
	...
	private void setUpIndexPanel(JPanel indexPanel) {
		...
		prevButton.setPreferredSize(new Dimension(prevIcon.getIconWidth() + 2, prevIcon.getIconHeight() + 2));
		firstButton.setPreferredSize(new Dimension(firstIcon.getIconWidth() + 2, firstIcon.getIconHeight() + 2));
		nextButton.setPreferredSize(new Dimension(nextIcon.getIconWidth() + 2, nextIcon.getIconHeight() + 2));
		lastButton.setPreferredSize(new Dimension(lastIcon.getIconWidth() + 2, lastIcon.getIconHeight() + 2));
	...
	public void checkScroll() {
		...
		if (sampleWidth != zoomOutWidth) {
			...
			if (currentPixelPosition < rectMinX || currentPixelPosition > rectMaxX) {
				...
				// calculate how to position the current position in the middle
				// of the viewing
				// area
	...
	private void handleZoomIn(boolean checkScrollFlag) {
		...
		if (DEBUG) {
			System.out.println("Zoom In:  currentPixelPosition = " + currentPixelPosition);
		}
	...
	private void handleZoomOut() {
		...
		if (DEBUG) {
			System.out.println("Zoom Out:  currentPixelPosition = " + currentPixelPosition);
		}
	...
	private void handleFramesPerPixel(int numFrames) {
		...
		soundPanel.setPreferredSize(new Dimension(sampleWidth, soundPanel.getHeight()));
		soundPanel.setSize(soundPanel.getPreferredSize());

		leftSoundPanel.setPreferredSize(new Dimension(sampleWidth, leftSoundPanel.getHeight()));
		leftSoundPanel.setSize(leftSoundPanel.getPreferredSize());

		leftSampleWrapper.setPreferredSize(new Dimension(sampleWidth, leftSampleWrapper.getHeight()));
		leftSampleWrapper.setSize(leftSampleWrapper.getPreferredSize());
		...
		if (inStereo) {
			rightSoundPanel.setPreferredSize(new Dimension(sampleWidth, rightSoundPanel.getHeight()));
			rightSoundPanel.setSize(rightSoundPanel.getPreferredSize());

			rightSampleWrapper.setPreferredSize(new Dimension(sampleWidth, rightSampleWrapper.getHeight()));
			rightSampleWrapper.setSize(rightSampleWrapper.getPreferredSize());
	...
	/*
	 * public static void main(String args[]) { try{ /* Sound s = new
	 * Sound("/Users/ellie/mediacomp/ellie/really_long_sound.wav");
	 * SoundExplorer test = new SoundExplorer(s, s.isStereo());
	 */

	// Sound s2 = new
	// Sound("/Users/ellie/mediacomp/ellie/SoundDemo/audio/22-new.aif");
	 ...
	 */
	/*
	 * Sound sStates = new Sound("Z:\\croak.wav"); SoundExplorer testStates =
	 * new SoundExplorer(sStates, sStates.isStereo());
	 */
	/*
