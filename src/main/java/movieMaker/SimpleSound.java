...
/**
 * The <code>SimpleSound</code> class is an implementation of the Java Sound API
 * specifically designed for use with students.
...
public class SimpleSound {
	...
	/**
	 * Constant for the default sample size in bits it is usual to have either 8
	 * or 16
	 */
	...
	/**
	 * Constructs a <code>SimpleSound</code> of the specified length. This sound
	...
	public SimpleSound(SimpleSound sound) {
		...
		if (sound.buffer != null) {
			buffer = new byte[sound.buffer.length];
			for (int i = 0; i < sound.buffer.length; i++) {
				buffer[i] = sound.buffer[i];
			}
		}
	}
	...
	public void loadFromFile(String inFileName) throws SoundException {
		 ...
		 * bytes we will be storing cannot be greater than Integer.MAX_VALUE.
	...
	/**
	 * Checks the value of durInFrames to make sure that it is not larger than
	 ...
	 * @see SimpleSound#playAtRateInRange(float rate, int startFrame, int
	...
	/**
	 * First, checks the value of durInFrames to make sure that it is not larger
	 ...
	 * @param durInFrames
	 *            a double representing how much of this sound we want to play
	 * @see SimpleSound#playAtRateInRange(float range, int startFrame, int
	 *      endFrame, boolean isBlocking)
	 * @throws SoundException
	 *             if there are problems playing the sound.
	 */
