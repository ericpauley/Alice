...
public class TitleBubble extends Bubble {
	...
	/* (non-Javadoc)
	 * @see edu.cmu.cs.stage3.alice.core.bubble.Bubble#paintBackground(java.awt.Graphics)
	 */
	...
	// narrate doesn't really have an origin, this is more to calculate a reasonable set of offsets
	...
	public void calculateBounds( edu.cmu.cs.stage3.alice.scenegraph.renderer.RenderTarget rt ) {
		...
		java.awt.geom.Rectangle2D totalBound = this.getTotalBound();
			
		this.setPixelOffset(new java.awt.Point((int)actualViewport.getX() + (int)(actualViewport.getWidth()/2 - totalBound.getWidth()/2), (int)actualViewport.getY() + (int)(actualViewport.getHeight()/2 - totalBound.getHeight()/2)));
	}
